...
Uses cross-language fallback for European languages.
"""
...
def build_set_mapping():
    ...
    # Also add direct transformations
    ptcg_sets = get_ptcg_sets()
    
    # Get all missing English set IDs
    ...
    for (set_id,) in missing_sets:
        ...
        if transformed in ptcg_sets:
            ...
            continue
        # Try prefix matching for numbered sets
        # e.g., B2a -> bw2, B1a -> bw1
        prefix = re.match(r'([a-z]+)(\d+)([a-z]?)', set_id.lower())
        if prefix:
            base, num, suffix = prefix.groups()
            # Try common prefixes
            for ptcg_prefix in ['bw', 'xy', 'sm', 'sv', 'swsh', 'dp', 'pl', 'hgss', 'ex', 'base', 'gym', 'neo', 'pop']:
...
def main():
    ...
    # Get all missing English cards grouped by set
    conn = sqlite3.connect(DB); cur = conn.cursor()
    rows = cur.execute('''
    ...
    for set_id, count in rows:
        if set_id not in mapping:
            print(f'{set_id} ({count}): NO MAPPING')
            continue
        
        ptcg_set_id = mapping[set_id]
        db_cards = get_missing_cards_for_set(set_id)
        
        print(f'{set_id} -> {ptcg_set_id} ({len(db_cards)} cards):', end=' ', flush=True)
        
        # Get all cards from pokemontcg.io set to build local_id -> image map
        ...
        try:
            ...
            for c in db_cards:
                ...
                if img_url:
                    ...
                    all_manifest.append({
                        'set_id': set_id, 'card_id': c['card_id'], 'language_code': 'en', 'status': 'card_not_found',
                    })
