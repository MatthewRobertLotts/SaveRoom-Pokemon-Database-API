...
def fetch(url): return urllib.request.urlopen(urllib.request.Request(url,headers=HEADERS),timeout=30).read().decode('utf-8','ignore')
def clean(s): return re.sub(r'\s+',' ',html.unescape(re.sub(r'<[^>]+>',' ',s))).strip()
def parse_name(raw,cid):
 m=re.search(r'<title>(.*?)</title>',raw,re.S|re.I)
 if not m: return None
 t=clean(m.group(1))
 if 'Pardon Our Interruption' in t: return None
...
def unresolved_pocket(conn):
 ...
 WHERE p.language_code='de' AND s.series_name LIKE '%Pocket%' AND p.method='cross_language_template'
 ...
 """).fetchall()
def get(row):
 cid=row['card_id']; url=f'https://pokemontcgpocket.app/de/card/{cid}'
 try:
...
def main():
 ...
 rows=unresolved_pocket(conn); print('de pocket unresolved',len(rows))
 results=[]
 ...
 for res in results:
  ...
  conn.execute("""insert or replace into card_source_provenance(language_code,set_id,card_id,source_url,method,note,fetched_at) values (?,?,?,?,?,?,?)""",('de',res['set_id'],res['card_id'],res['url'],'pokemontcgpocket_app_de_card_page',f'German Pocket name parsed from pokemontcgpocket.app German card page; old fallback={res["old_name"]!r}.',FETCHED_AT))
  updated+=1
  if len(examples)<70: examples.append({'card_id':res['card_id'],'old':res['old_name'],'new':res['name'],'url':res['url']})
 conn.commit()
 ...
 report={'fetched_at':FETCHED_AT,'input_rows':len(rows),'updated':updated,'failure_count':len(failures),'remaining_de':sum(x[1] for x in by_set),'remaining_by_set':by_set,'examples':examples,'failures':failures[:80]}
 out=REPORT_DIR/'pokemontcgpocket_app_de_completion.json'; out.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
