...
def unresolved_pocket(conn):
 ...
 WHERE p.language_code='es' AND s.series_name LIKE '%Pocket%' AND p.method='cross_language_template'
 ...
 """).fetchall()
def get(row):
 cid=row['card_id']; url=f'https://pokemontcgpocket.app/es/card/{cid}'
 try:
...
def main():
 ...
 rows=unresolved_pocket(conn); print('es pocket unresolved',len(rows))
 results=[]
 ...
 for res in results:
  ...
  conn.execute('update cards set name=? where language_code=? and card_id=?',(res['name'],'es',res['card_id']))
  conn.execute("""insert or replace into card_source_provenance(language_code,set_id,card_id,source_url,method,note,fetched_at) values (?,?,?,?,?,?,?)""",('es',res['set_id'],res['card_id'],res['url'],'pokemontcgpocket_app_es_card_page',f'Spanish Pocket name parsed from pokemontcgpocket.app Spanish card page; old fallback={res["old_name"]!r}.',FETCHED_AT))
  updated+=1
 ...
 report={'fetched_at':FETCHED_AT,'input_rows':len(rows),'updated':updated,'failure_count':len(failures),'remaining_es':remaining,'remaining_by_set':by_set,'examples':examples,'failures':failures[:50]}
 out=REPORT_DIR/'pokemontcgpocket_app_es_completion.json'; out.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
 print(json.dumps(report,ensure_ascii=False,indent=2)[:14000]); print('Report:',out)
 conn.close()
