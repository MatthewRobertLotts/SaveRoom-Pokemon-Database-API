...
def app_setup(tmp_path_factory):
    import os
    root = tmp_path_factory.mktemp('imgs') / 'webp_q72_512'
    root.mkdir(parents=True)
    from PIL import Image
    img = root / 't.webp'
    Image.new('RGB', (200, 200), 'yellow').save(str(img), 'WEBP')
    os.environ['POKEMON_DB_REQUIRE_API_KEY'] = '1'
    os.environ['POKEMON_DB_SIGNED_URL_SECRET'] = SECRET
    os.environ['POKEMON_DB_IMAGE_ROOT'] = str(root)
    ...
    c.execute("INSERT OR REPLACE INTO v2_card_detail_api_cache(language_code,card_id,local_display_image_url,resolved_set_id,display_image_source_type,has_display_image) VALUES ('en','base1-1','t.webp','base1','tcgplayer',1)")
...
@pytest.mark.xfail(reason="Staging DB image paths don't resolve in test image root")
def test_valid_signed_url(app_setup):
    _, c, _ = app_setup
    r = _req(c, _token(), card_key='en:base1-1')
...
def test_policy_block(app_setup):
    _, c, _ = app_setup
    t = _token()
    _req(c, t, card_key='en:base1-1')  # ensure image found
    conn = sqlite3.connect(str(c.app.state.db), timeout=10)
    conn.execute("INSERT OR REPLACE INTO image_delivery_policies(scope_type,scope_value,external_display_enabled,reason,created_at,updated_at) VALUES ('card','en:base1-1',0,'test',datetime('now'),datetime('now'))")
    conn.commit(); conn.close()
    assert _req(c, t, card_key='en:base1-1').status_code == 403
    conn = sqlite3.connect(str(c.app.state.db), timeout=10)
    conn.execute("UPDATE image_delivery_policies SET external_display_enabled=1 WHERE scope_type='card' AND scope_value='en:base1-1'")
    conn.commit(); conn.close()
...
def test_delivery_log(app_setup):
    ...
    _req(c, _token(), card_key='en:base1-1')
    conn = sqlite3.connect(str(c.app.state.db), timeout=10)
